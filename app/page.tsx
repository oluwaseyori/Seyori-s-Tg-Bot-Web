...
const COMMANDS: CommandBlock[] = [
  { plugin: 'Core', summary: 'General commands anyone can use to discover the bot.', rows: [
  ...
  { plugin: 'Donate (Stars)', summary: 'Support the project with Telegram Stars (XTR).', rows: [
  ...
  { plugin: 'Games', summary: 'Party games with smart rules and stats.', rows: [
  ...
  { plugin: 'Group Admin (gcadmin)', summary: 'Admin utilities for managing groups. Bot must be admin with proper rights.', rows: [
  ...
  { plugin: 'Media tools', summary: 'Convenience utilities for images and file hosting.', rows: [
  ...
  { plugin: 'Planned / Coming soon', summary: 'Upcoming features on the roadmap.', rows: [
    ...
    <main className="min-h-screen bg-black text-green-400">
      ...
      <section id="commands" className="mx-auto max-w-6xl px-4 py-12">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          {/* Filters: always visible, mobile-friendly grid */}
          <div className="grid w-full grid-cols-1 gap-2 sm:w-auto sm:grid-cols-3">
            <select value={pluginFilter} onChange={e => setPluginFilter(e.target.value)} className="rounded-xl border border-green-800 bg-black px-3 py-2 text-sm text-green-300 focus:outline-none">
              {['All', ...COMMANDS.map(b => b.plugin)].map(p => <option key={p} value={p} className="bg-black text-green-300">{p}</option>)}
            </select>
            <select value={roleFilter} onChange={e => setRoleFilter(e.target.value as any)} className="rounded-xl border border-green-800 bg-black px-3 py-2 text-sm text-green-300 focus:outline-none">
              {['All','any','admin','owner'].map(p => <option key={p} value={p} className="bg-black text-green-300">{p}</option>)}
            </select>
            <select value={scopeFilter} onChange={e => setScopeFilter(e.target.value as any)} className="rounded-xl border border-green-800 bg-black px-3 py-2 text-sm text-green-300 focus:outline-none">
